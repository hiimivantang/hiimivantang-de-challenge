...
At the center of the Star Schema is our fact table, which in our case is the `sales_details` table that stores the line items for each transaction.
Note that a single transaction can consist of multiple records in `sales_details` table as a customer is able to buy multiple items with a single order.
